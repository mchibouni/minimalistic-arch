<div class="wrap slider-pro">
			...
			echo '<tr>'.
					...
					'<td>' .
						  ...
						  if (get_option('slider_pro_generate_xml_file')) {
							  global $blog_id;
							  $name = (function_exists('is_multisite') && is_multisite()) ? $slider_name . ' (' . $blog_id . '-' . $slider->id . ')' : $slider_name . ' (' . $slider->id . ')';
							  
							  echo ' | <a href="' . plugins_url('/slider-pro/includes/export.php?name=' . $name) . '">' . __('Export', 'slider_pro') . '</a>';
						  }
    ...
    <div id="new-slider-button">    
        ...
        <a class="button-secondary" id="import-slider" href="<?php echo admin_url('admin-ajax.php?action=sliderpro_slider_import'); ?>"><?php _e('Import a Slider', 'slider_pro'); ?></a>
    </div>    
